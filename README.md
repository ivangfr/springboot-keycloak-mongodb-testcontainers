...
1. Create a REST API application to manage books, `book-service`. The data is stored in a `MongoDB` database.
...
#### 1) Running `init_keycloak.sh` script
...
*PS. Update the `BOOKSERVICE_CLIENT_SECRET` value with the secret generated by `Keycloak` (Configuring Keycloak)*
