...
- In a terminal and inside `springboot-testing-mongodb-keycloak` root folder, run the following script to initialize
  `Keycloak`
   ```
   ./init-keycloak.sh
   ```
...
- `BOOKSERVICE_CLIENT_SECRET` value is shown at the end of the script outputs. It will be needed whenever we call
...
1. Run the script below to get an access token for `ivan.franchin` user
   ```
...
1. Get the access token as explained at [Getting Access Token](https://github.com/ivangfr/springboot-testing-mongodb-keycloak#getting-access-token)
...
1. Export to `BOOKSERVICE_CLIENT_SECRET`, the `Client Secret` generated by `Keycloak`
   ```
   export BOOKSERVICE_CLIENT_SECRET=...
   ```

1. In order to get the access token from `Keycloak`, run the following command. Note that we are informing `localhost`
