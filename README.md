...
1. Create a REST API application to manage books, `book-service`. The data is stored in a `MongoDB` database. The application will have its endpoints related to add/update/delete books secured.
...
*PS. Update the `BOOKSERVICE_CLIENT_SECRET` value with the secret generated by Keycloak (Configuring Keycloak, step 4)*
